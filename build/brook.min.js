...
return ns.promise(function(next,val){next(value);});};var EMIT_INTERVAL_MAP={};var emitInterval=function(msec,name){var msecFunc=(typeof msec=='function')?msec:function(){return msec};return ns.promise(function(next,val){var id=setInterval(function(){next(val);},msecFunc());if(name){EMIT_INTERVAL_MAP[name]=id;}});};var stopEmitInterval=function(name){return ns.promise(function(next,value){clearInterval(EMIT_INTERVAL_MAP[name]);next(value);});};var parallel=function(){var promises=Array.prototype.slice.call(arguments);return ns.promise(function(next,value){var count=0;for(var i=0;i<promises.length;i++){var promise=promises[i];promise.bind(ns.promise(function(n,v){n(v);count++;if(count>=promises.length){next(v);}})).run(value);}});};ns.provide({mapper:mapper,through:through,filter:filter,scatter:scatter,takeBy:takeBy,wait:wait,cond:cond,match:match,debug:debug,lock:lock,unlock:unlock,from:from,parallel:parallel,waitUntil:waitUntil,emitInterval:emitInterval,stopEmitInterval:stopEmitInterval});});Namespace('brook.lambda')
...
this.remove=function(token){check(token);for(var i=0;i<this._classList.length;++i){if(this._classList[i]==token){this._classList.splice(i,1);}}
this._element.className=this._classList.join(" ");this.length=this._classList.length;}
